...
class DynamicGeoJson(MacroElement):
    ...
    ***Experimental***
    ...
    nE = latlng._northEast.{{ this.order[0] }} + "," + latlng._northEast.{{ this.order[1] }}
    sW = latlng._southWest.{{ this.order[0] }} + "," + latlng._southWest.{{ this.order[1] }}
    var bbox = sW + d + nE
    console.log(bbox);


    // This will make a new rest api call.
    var url = "{{ this.url_root }}" + bbox;
